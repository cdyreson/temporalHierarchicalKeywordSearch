 ...
 * @author truongbaquan
 */
public class BufferedCompleteFSLCAFinder extends SLCAFinder {
    ...
    public BufferedCompleteFSLCAFinder(/*BinaryDeweyUtils nodeUtils, */TermNodeIndex termNodeIndex) {
        ...
        try (Cursor cursor = termNodeIndex.openCursor()) {
            ...
            for (NodeId node : list) {
                ...
                if (lastLca.lessThanOrEqualTo(lca)) {
                    if (!lastLca.isAncestorOrDescendant(lastLca))
                        result.add(lastLca);
                    lastLca = lca;
                }           
            }
