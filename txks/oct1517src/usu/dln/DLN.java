 ...
 * written as a 1-bit.
 */
public class DLN implements PathId, NodeId, Comparable, Serializable {
    ...
    protected transient static final DLN ROOT_NODE = new DLN(1);
    // Establish a node that is last in the ordering, greater than the root node
    protected transient static final DLN MAX_NODE = new DLN(2);
    // Establish a node that is first in the ordering, greater than the root node
    protected transient static final DLN MIN_NODE = new DLN(0);
     ...
     */
    public DLN(short units, byte[] data, int startOffset) {
        if (units < 0) {
    ...
    protected DLN(byte[] data, int length, short index) {
    ...
    protected DLN(byte[] data, short nbits) {
     ...
     */
    public DLN commonLCA(DLN other, int level) {
        ...
        for (int i = level; i >= 1; i--) {
            ...
            if (compareBitsExact(other, bitCount)) {
                return new DLN(this.bits, (short) bitCount);
            }
        }
        return new DLN(this.bits, (short) getStartBitAtLevel(0));
     ...
     */
    public boolean hasCommonLCA(DLN other, int level) {
        //level = (level == 0) ? 0 : level - 1;
    ...
    public boolean equals(DLN other) {
...
//    }
    public int compareBits(DLN other, int bitCount) {
     ...
     * @param other
     */
    public boolean compareBitsExact(DLN other, int bitCount) {
        int bytes = bitCount / 8;
        int remaining = bitCount % 8;
        for (int i = 0; i < bytes; i++) {
            if (bits[i] != other.bits[i]) {
     ...
     * @param other
     */
    public boolean startsWith(DLN other) {
     ...
     */
    @Override
    public DLN insertNode(NodeId right) {
        return insertNode((DLN) right);
    }

    @Override
    public DLN insertNode(PathId right) {
        return insertNode((DLN) right);
    }

    public DLN insertNode(DLN right) {
...
//            System.out.println("newNode: " + newNode.toString() + " = " + newNode.bitIndex + "; last = " + lastPos);
    ...
    public DLN getParentId() {
        ...
        return new DLN(bits, (short) (last - 1));
    }
    ...
    public boolean isDescendantOf(NodeId ancestor) {
        return isDescendantOrSelfOf((DLN) ancestor);
    }
    ...
    public boolean isDescendantOf(DLN other) {
    ...
    public boolean isDescendantOrSelfOf(DLN ancestor) {
    ...
    public boolean isChildOf(DLN other) {
        if (!startsWith(other)) {
            return false;
        }
    ...
    public int computeRelation(NodeId other) {
        return computeRelation((DLN) other);
    }
    ...
    public int computeRelation(DLN other) {
        if (other == ROOT_NODE) {
    ...
    public boolean isSiblingOf(NodeId sibling) {
        return isSiblingOf((DLN) sibling);
    }
    ...
    public boolean isSiblingOf(DLN sibling) {
        //DLN other = (DLN) sibling;
        DLN parent = getParentId();
     ...
     */
    public int compareTo(DLN otherId) {
        //System.out.println("compareTo");
        if (otherId == null) {
            return 1;
        }
        //System.out.print("Comparing " + this + " to " + otherId);
        final DLN other = otherId;
        final int a1len = bits.length;
        final int a2len = other.bits.length;
    ...
    public boolean after(NodeId other, boolean isFollowing) {
        return after((DLN) other, isFollowing);
    }
    ...
    public boolean after(DLN other, boolean isFollowing) {
    ...
    public boolean before(NodeId other, boolean isPreceding) {
        return before((DLN) other, isPreceding);
    }
    ...
    public boolean before(DLN other, boolean isPreceding) {
        if (compareTo(other) < 0) {
            if (isPreceding) {
     ...
     */
    public static DLN getRootId() {
        return ROOT_NODE;
    }

    public static DLN getMaxId() {
        return MAX_NODE;
    }

    public static DLN getMinId() {
        return MIN_NODE;
    }

    @Override
    public int computeNCALevel(PathId other) {
        return computeNCALevel((DLN) other);
    }
     ...
     * @return common lca, or null if none
     */
    public int computeNCALevel(DLN other) {
    ...
    public int computeNCALevel(int tlevel, PathId other, int olevel) {
        return computeNCALevel(tlevel, (DLN) other, olevel);
    }
    ...
    public int computeNCALevel(int tlevel, DLN other, int olevel) {
        int level = (tlevel < olevel) ? tlevel : olevel;
        //level = (level == 0) ? 0 : level - 1;
        for (int i = level; i >= 1; i--) {
            int bitCount = getStartBitAtLevel(i) - 1;
     ...
     */
    public boolean isAncestor(DLN anc) {
    ...
    public boolean isAncestorOrDescendant(DLN anc) {
     ...
     */
    public DLN getAncestor(int level) {
        if (level == 1) {
            return ROOT_NODE;
        }
        ...
        //return new DLN(bits, last - level);
        return new DLN(bits, (short) (getStartBitAtLevel(level) - 1));
    }
     ...
     */
    @Override
    public DLN getNextFirstDescendant(int level) {
        if (level <= 0) {
            throw new IllegalArgumentException();
        }
        DLN anc = this.getAncestor(level);
    ...
    public boolean lessThan(DLN other) {
    ...
    public boolean isEqualTo(DLN other) {
    ...
    public boolean isNotEqualTo(DLN other) {
        return !this.equals(other);
        //return (value != other.value);
    }
    ...
    public boolean lessThanOrEqualTo(PathId other) {
        return lessThanOrEqualTo((DLN) other);
    }
    ...
    public boolean lessThanOrEqualTo(DLN other) {
    ...
    public static void main(String[] args) {
        ...
        DLN id4 = (DLN) id0.newChild();
        DLN id5 = (DLN) id0.nextSibling();
