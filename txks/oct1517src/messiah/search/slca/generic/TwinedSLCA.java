...
import messiah.search.slca.generic.feed.TemporalFeedGroup;
import messiah.search.slca.generic.feed.TemporalFeedsController;
 ...
 * @author curtis
 */
public class TwinedSLCA extends SLCAFinder {

    private final Database db;
    private final TemporalFeedsController controller;
    private int maxLevel = 0;

    boolean verbose = true;

    public TwinedSLCA(Database db) {
        this.db = db;
        controller = new TemporalFeedsController(db);
    }
                        ...
                        for (int i = level - 1; i >= 1; i--) {
                            if (slcas[i] != null) {
                                if (slca.isDescendantOf(slcas[i])) {
                                    if (verbose) {
                                        System.out.println("Modifying excludes " + i + " " + level + " slca " + slca + " slca[i] " + slcas[i]);
                                    }
