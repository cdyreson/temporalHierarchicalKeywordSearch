...
/**
 * The Feeds Level Controller maintains levels at which nodes are checked. A
 * level collects LCAs of a given type.
 *
 * @author Curt
 */
public class TemporalLevelFeedsController implements Iterator<NodeId> {
    ...
    public TemporalLevelFeedsController(Database db) {
        this.db = db;
    }
    ...
    private void wrapCreateTypeSLCAs(short dimensions, Set<PathId>[] paths) {
        ...
        for (PathId pathId : tempMap.keySet()) {
            System.out.println("Controller SLCA " + pathId);
        }
    }
