...
import messiah.search.slca.generic.algebra.*;
import java.util.*;
import usu.NodeId;
...
/**
 *
 * A FragmentFeed implements generates a node at a given level (the lowest level
 * at which it produces an LCA for the search) from the sequence of nodes that
 * match a given keyword
 *
 * @author Curt
 */
public class FragmentFeed implements Operator {

    private boolean verbose = false;
    // Internal sequence of nodes that match the keyword
    ...
    public FragmentFeed(Iterator<NodeId> iter, int level) {
        if (verbose) {
            System.out.println("FragmentFeed new " + level);
        }
        this.iter = iter;
    ...
    /*
     * Advance the iterator to the next node.  Just advances the iterator, does
     * not generate the node.
     */
    private void advance() {
        if (verbose) {
            System.out.println("FragmentFeed advance ");
        }
        if (!iter.hasNext()) {
            if (verbose) {
                System.out.println("FragmentFeed advance is empty! ");
            }
            empty = true;
            return;
        }
