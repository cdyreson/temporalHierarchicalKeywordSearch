...
public class TemporalFeedsController implements Iterator<NodeId> {
    ...
    private Database db;
    private Map<PathId, Set<Integer>> typeSLCAMap;
    private int typeSLCACount = 0;
    ...
    private void wrapCreateTypeSLCAs(short dimensions, Set<PathId>[] paths) {
        if (verbose) {
            System.out.println("TemporalFeedsController: Wrap ");
        }
        ...
        if (verbose) {
            for (PathId pathId : tempMap.keySet()) {
                System.out.println("TemporalFeedsController SLCA " + pathId + " " + tempMap.get(pathId));
            }
        }
    }
    ...
    private void createTypeSLCAs(short current, short dimensions, PathId pathLCA, int hashKey, Set<PathId>[] paths) {
        if (verbose) {
            System.out.println("TemporalFeedsController: CreateTypeSLCAs: " + current + " " + dimensions);
        }
        // Reached the end
        if (current >= dimensions) {
            if (verbose) {
                System.out.println("TemporalFeedsController: level is " + pathLCA.getLevel());
            }
        ...
        for (CachedNodeTriple trip : whoIsNext) {
            ...
            if (tempLCA == null) {
                ...
                if (!tempLCA.equals(lca2)) {
                    ...
                    if (verbose) {
                        System.out.println("TemporalFeedsController: no match");
                    }
